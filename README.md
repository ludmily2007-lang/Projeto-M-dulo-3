...
Responsabilidades: Desenvolvimento das funcionalidades, organização do sistema, testes da aplicação e correções técnicas.

Integrante: Ludmily Lorrany
Função: Design & UX
Responsabilidades: Definição visual do sistema, revisão de layout, organização das telas, testes de usabilidade e registros do protótipo.

Integrante: Briza Cavalcanti
Função: Pesquisa & Documentação
Responsabilidades: Pesquisa sobre plataformas low-code/no-code, redação da documentação, análise crítica e organização do repositório.

Integrante: Herisson Wellington
Função: Suporte Técnico & Revisão
Responsabilidades: Auxílio na configuração da plataforma, revisão das funcionalidades, testes finais e apoio na validação do sistema.
